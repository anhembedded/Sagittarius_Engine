...
| `tools/` | Standalone utilities built on top of the engine, not part of the installable package. `widget_showcase/` — a PySide6 gallery of the `pyside_mvc` widget kit. (`audit_dashboard/` was here until `EPIC-005A` deleted it; `sagittarius-trace` replaces it as a console script, not a tool directory.) |

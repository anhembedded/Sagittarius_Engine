...
- **Status**: 🟡 In Progress — **E0 done 2026-09-19** (`ScheduledJob.cancel()`), **E1 landed
  ...
  `IContributionRegistry`, `ContributionRegistry`), **E2 landed 2026-09-19** (`RegionKind`,
  `IRegionHost`, `RegionHost` — §"How the consumer feeds this task" table below). E3's sequencing
  is decided (harvest-first — see the note under the table) but not started: it is triggered by the
  consumer's own Phase 5 landing a working `NavigationService`-shaped mechanism in its own tree
  first, which that phase has not yet begun. Consumer's Elite `EPIC-025` Phases 1–4 (which E1/E2
  drew from) are done; Phase 5 (which E3 waits on) has not started.
...
| E2 | after its Phase 2 | ✅ **Landed 2026-09-19.** `sagittarius_engine/extensions/pyside_mvc/runtime/`: `region_kind.py` (`RegionKind` — the engine-owned, closed `QMainWindow` anatomy: `TOP_TOOLBAR`/`SECONDARY_TOOLBAR`/`CENTRAL`/`DOCK_LEFT`/`DOCK_RIGHT`/`DOCK_TOP`/`DOCK_BOTTOM`/`STATUS_BAR`/`MODAL`), `i_region_host.py` (`IRegionHost`, a `Protocol` — harvested from the consumer's `IPlaceHost`), `region_host.py` (`RegionHost` — harvested from the consumer's `WorkbenchSurface`, in production on all six of that app's surfaces since `PR 1.4`, unchanged through four phases). Two changes from a verbatim copy: `place` is an opaque `str` (same fix as `ContributionDescriptor.place` in E1), and *where* a place renders is supplied by the caller as `place_regions: Mapping[str, RegionKind]` at construction (mandatory, no default — the consumer's own registry gap E1 already fixed, applied here from the start). Deliberately not harvested: the consumer's environment-banner slot — shell chrome specific to that app, not a place slot this table names, same YAGNI call as `Surface.owner`/`gated_by` in E1. **This exposed a design-constraint drift in `EPIC-001D`'s own file** ("Python describes, QML renders"; "registry state as models") — written before this epic had a real consumer, and that consumer decided QtWidgets-only, no QML (ADR D20–D22, the consumer's own decision, 2026-09-13). Noted directly in `EPIC-001D`'s own file rather than silently landing code that contradicts it: those two constraints are marked inapplicable to this harvested slice, not deleted (a future QML-rendering consumer would still need them). 22 new tests in `tests/extensions/pyside_mvc/test_region_host.py`, mirroring the consumer's own `test_workbench_surface.py` coverage; mutation-verified (the construction-time coverage check and the second-central-widget refusal each forced to `if False:` in turn, confirmed the matching tests fail for the right reason, restored — `git diff --stat` clean). `ruff`/`mypy` clean (233 files); full `tests/extensions/pyside_mvc/` suite green (199 passed) |

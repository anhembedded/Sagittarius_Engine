...
| ID | Title | Severity | Reported | Note |
| :--- | :--- | :---: | :---: | :--- |
| **[BUG-014](completed/BUG-014_intermittent_segfault_under_scheduler_and_asyncruntime_thread_pressure_in_full_suite.md)** | Intermittent `Segmentation fault` in the full test suite, ~100 leaked `Scheduler`/`AsyncRuntime` threads at crash time | Medium | 2026-09-19 | Fixed 2026-09-19: `Scheduler.stop()`/`AsyncRuntime.stop()` unconditionally cleared their thread reference (and `AsyncRuntime` closed its loop) after `join(timeout=...)`, whether or not the join actually succeeded — a thread still alive past the deadline was silently discarded rather than reported, and closing a loop still running on its own thread is undefined behaviour in `asyncio`. Both now check `is_alive()` and leave a still-running thread tracked instead. 2 mutation-verified regression tests; 3 consecutive clean full-gate runs post-fix (was 2 segfaults in 5). The separate `gc: 45 uncollectable objects` `ResourceWarning` persists and is **not** claimed fixed. Surfaced `BUG-015` (unrelated) along the way. |

...
| :--- | :--- | :---: | :---: | :--- |
| **[BUG-014](incomplete/BUG-014_intermittent_segfault_under_scheduler_and_asyncruntime_thread_pressure_in_full_suite.md)** | Intermittent `Segmentation fault` in the full test suite (~1 in 3 runs), ~100 leaked `Scheduler`/`AsyncRuntime` threads at crash time | Medium | 2026-09-19 | Found verifying `TASK-043` E1's gate; reproduced with that change's files entirely absent (`git stash`), so it predates and is independent of the harvest. Crashes at the same ~94% progress point, right after `test_examples.py`'s stress tests, in every trace collected. |

...
| **TASK-042** | `create_quick_widget()` clears every `QQuickWidget` to an opaque, token-driven background — `setClearColor(transparent)` only "shows the parent through" on the software path; real desktops rendered black/see-through (consumer `BUG-115`) | UI Engine / Runtime (`pyside_mvc`) | 2026-09-10 | [TASK-042_quick_widget_opaque_token_background.md](completed/TASK-042_quick_widget_opaque_token_background.md) |
